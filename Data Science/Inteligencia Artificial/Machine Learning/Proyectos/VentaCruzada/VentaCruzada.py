...
def deal_duplicates_ventaCruzada(df, unify=True):
    ...
    if id != desc:
        ...
        if ids < desc:
            ...
            if unify == True:  # OPCION A: UNIFICAR DUPLICADOS
                ...
                par = par.sort_values(
                    by=['IdProducto', 'Producto'], ascending=True)
    ...
    if ids == desc:
        print(' ')
        # Validamos duplicados
        print('Completado, se han removido exitosamente todos los duplicados faltantes')
...
def matriz(df, id=True):
    ...
    # salto de linea, sin espacios adelante para que funcione
    if id == True:
        return df.groupby(['Key', 'IdProducto'])['Cantidad'].sum().unstack().fillna(0). \
            applymap(lambda x: 1 if x >
                     0 else 0)  # devolvera escalar por cada variable
    else:
        return df.groupby(['Key', 'Producto'])['Cantidad'].sum().unstack().fillna(0). \
            applymap(lambda x: 1 if x >
                     0 else 0)  # usamos las descripciones, poco comun
